...
function formattingOfInitials(fullName) {
  ...
  const arrNew = [];
  arrNew.push(arr[1].slice(0, 1));
  arrNew.push(arr[2].slice(0, 1));
  arrNew.push(arr[0]);
  return arrNew.join(". ");
}
...
function setContractorHome() {
  if (!EdocsApi.getAttributeValue("OrganizationId").value) {
    try {
      const data = EdocsApi.getContractorByCode("40081293", "homeOrganization");
      //EdocsApi.setAttributeValue({ code: "Organization", value:data. });
      ...
      if (data.authorisedPersons.length > 0) {
        EdocsApi.setAttributeValue({ code: "OrgAgent1Id", value: data.authorisedPersons[0].contractorId });
        EdocsApi.setAttributeValue({ code: "OrgAgentSurname1", value: data.authorisedPersons[0].nameGenitive });
        EdocsApi.setAttributeValue({ code: "OrgAgent1", value: data.authorisedPersons[0].fullName });
        EdocsApi.setAttributeValue({ code: "OrgAgentPosition", value: data.authorisedPersons[0].position });
        EdocsApi.setAttributeValue({ code: "ActsOnBasisOrg", value: data.authorisedPersons[0].actingUnderThe });
      }

      if (data.contacts.length > 0) {
        EdocsApi.setAttributeValue({ code: "TelephoneOrg", value: conInfo.contacts.find(x => x.type == "Phone")?.contact });
        EdocsApi.setAttributeValue({ code: "EmailOrg", value: conInfo.contacts.find(x => x.type == "Email")?.contact });
      }
    } catch (error) {
